...
public interface IArfatpDao extends PagingAndSortingRepository<Arfatp,IdArfatp> {
	...
	@Query("SELECT a FROM Arfatp a  WHERE a.idArfa.cia = :cia AND a.indPTota = :pvta")
	List<Arfatp> findAll(@Param("cia") String cia, @Param("pvta") String pvta);
	
	@Query("SELECT a FROM Arfatp a  WHERE a.idArfa.cia = :cia AND a.idArfa.tipo = :tipo AND a.indPTota = :pvta")
