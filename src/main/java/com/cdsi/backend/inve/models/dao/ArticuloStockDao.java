...
public interface ArticuloStockDao extends PagingAndSortingRepository<StockLibroDTO, IdArticulo> {
	...
	@Query(value="SELECT PRECIO, STOCK  FROM ARINDA1_VIEW WHERE NO_CIA = :cia AND ALMACEN = :alm AND TIPO = :pre AND NO_ARTI=:arti",nativeQuery = true)
	List<Object[]> traePrecStock(@Param("cia") String cia, @Param("alm") String alm, @Param("pre") String pre,@Param("arti") String arti);
