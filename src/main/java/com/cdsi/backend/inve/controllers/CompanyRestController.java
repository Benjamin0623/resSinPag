...
public class CompanyRestController {
	...
	public ResponseEntity<?> show(@PathVariable String cia) {
		...
		try {
			arfamc = compaService.findArfamc(cia);
		...
		if(arfamc == null) {
			response.put("mensaje", "La empresa con el id: ".concat(cia.concat(" no existe en la base de datos!")));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.NOT_FOUND);
		}
		
		return new ResponseEntity<Arfamc>(arfamc, HttpStatus.OK);
	}
